...
class VideoDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # self.data = self.data[:8] # for cq7 training
    ...
    def load_video(self, file_path):
        ...
        # remain_index_boundary = frame_count - num_frames - 1
        # if remain_index_boundary < 1:
        #     remain_index_boundary = 1
        # begin_index = random.randint(1, remain_index_boundary) # Get the start timestamp of the training video.
        if frame_count > 100:
            begin_index = random.randint(9, 39)
        elif frame_count > 60:
            begin_index = random.randint(1, 9)
        else:
            begin_index = 1 # Speed up the training.
        
        frames = []
        for frame_id in range(begin_index, num_frames + begin_index):
            frame = reader.get_data(frame_id)
            frame = Image.fromarray(frame)
            frame = self.crop_and_resize(frame, *self.get_height_width(frame))
            frames.append(frame)
        reader.close()
        return frames, begin_index
    ...
    def resize_list(self, array:list, length: int):
    ...
    def __getitem__(self, data_id):
        ...
        try:
            # video_path = os.path.join(self.base_path, data["video"])
            # camera_noise_path = os.path.join(self.base_path, data["camera_noise"])
            video_path = data["video"]
            camera_noise_path = data["camera_noise"]
            caption = data["prompt"]
            
            data = {}
            data["prompt"] = caption
            video, begin_index = self.load_data(video_path)
            
            camera_noise = np.load(camera_noise_path)
            camera_noise = torch.tensor(camera_noise)
            camera_noise = einops.rearrange(camera_noise, 'F H W C -> F C H W') # shape: [49, 16, 160, 160]
            camera_noise = camera_noise[begin_index : (begin_index + 49), :, :, :] # get the corresponding noise sequence.
            
            # assert camera_noise.shape[-1] == 128 and camera_noise.shape[-2] == 72
            
            data["video"] = video
            data["camera_noise"] = camera_noise
        except:
            print(f"********** {data_id} has no data. **********")
        return data
...
class ModelLogger:
    ...
    def on_epoch_end(self, accelerator, model, epoch_id):
        ...
        if accelerator.is_main_process:
            ...
            print(f"The models are saved at {path}")
...
def wan_parser():
    ...
    parser.add_argument("--serve_name", type=str, default="", help="Gradient accumulation steps.")
    parser.add_argument("--lora_ckpt_pth", type=str, default=None, help="Gradient accumulation steps.")
    return parser
